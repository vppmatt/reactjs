...
export const getAllPaymentsFetchVersion = () => {
    console.log("getting data using fetch");
    return fetch("http://localhost:8080/api/transaction",
        { method: "GET",
...
export const getAllPaymentsAxiosVersion = () => {
    ...
    return axios({url : "http://localhost:8080/api/transaction",
        method: "GET",
        headers: {'Accept' : 'application/json'} });
};

export const getAllPaymentsForCountry = (country) => {
    const transactionsPromise = axios({url : `http://localhost:8080/api/transaction?country=${country}`,
        method: "GET", headers : {'Accept': 'application/json'} });
    return transactionsPromise;
}

export const getAllPaymentsForOrderId = (orderId) => {
    const transactionsPromise = axios({url : `http://localhost:8080/api/transaction?orderId=${orderId}`,
        method: "GET", headers : {'Accept': 'application/json'} });
    return transactionsPromise;
}
